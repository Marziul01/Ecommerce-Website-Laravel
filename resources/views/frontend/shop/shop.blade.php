                    ...
                    <div class="shop-product-fillter">
                        ...
                        <div class="sort-by-product-area">
                            ...
                            <div class="sort-by-cover">
                                <select name="sort" id="sort" class="form-control m-0 me-2">

                                    <option value="latest" {{ $sort == 'latest' ? 'selected' : '' }}>Latest</option>
                                    <option value="featured" {{ $sort == 'featured' ? 'selected' : '' }}>Featured</option>
                                    <option value="low" {{ $sort == 'low' ? 'selected' : '' }}>Price: Low to High
                                    </option>
                                    <option value="high" {{ $sort == 'high' ? 'selected' : '' }}>Price: High to Low
                                    </option>
                                    ...
                                    @foreach ($products as $product)
                                        <div class="sf-column product-card">
                                            <div class="sf__pcard sf__pcard--onsale cursor-pointer sf-prod__block sf__pcard-style-4"
                                                data-view="card" data-product-id="8766631903454">
                                                ...
                                                <div class="text-center">
                                                    <div class="mt-3 lg:mt-5">
                                                        ...
                                                        <div class="sf__pcard-price leading-normal">
                                                            <div
                                                                class="f-price inline-flex justify-content-center flex-wrap f-price--on-sale text-center">
                                                                ...
                                                                @if ($product->productVariations->count() > 0)
                                                                    @php
                                                                        ...
                                                                        $sellingPrices = $variations->map(function (
                                                                            $v,
                                                                        ) {
                                                                            return $v->compare_price &&
                                                                                $v->compare_price > 0
                                                                                ? $v->compare_price
                                                                                : $v->price;
                                                                        });
                                                                    ...
                                                                    @if ($hasSale)
                                                                        ...
                                                                        <span
                                                                            class="text-muted text-decoration-line-through sale-price-range originalPrice">
                                                                            {{ number_format($minOriginal) }} -
                                                                            {{ number_format($maxOriginal) }}
                                                                        </span>

                                                                        <span class="fw-bold price-range sellingPrice">
                                                                            {{ number_format($minSelling) }} -
                                                                            {{ number_format($maxSelling) }} BDT
                                                                        </span>
                                                                    @else
                                                                        <span class="fw-bold price-range sellingPrice">
                                                                            {{ number_format($minSelling) }} -
                                                                ...
                                                                @else
                                                                    ...
                                                                    @if ($product->compare_price && $product->compare_price > 0)
                                                                        ...
                                                                        <span
                                                                            class="text-muted text-decoration-line-through sale-price originalPrice">
                                                        ...
                                                        <form method="POST" class="mt-2"
                                                                        ...
                                                                        <div>
                                                                            <ul class="list-filter size-filter font-small"
                                                                                ...
                                                                                @foreach ($product->productVariations as $variation)
                                                                                    <li>
                                                                                        <label
                                                                                            class="size-label variation-option"
                                                            ...
                                                            <div class="sf__pcard-action-atc flex justify-center mt-2">
                                                                ...
                                                                <button type="button" aria-label="Add To Cart"
                                                                    ...
                                                                    class="btn btn-sm btn-primary action-btn hover-up">Quick
                                                                    Add </button>
                                                    ...
                                                    @else
                                                        <button aria-label="Out of Stock !"
                                                            class="btn btn-danger btn-sm action-btn hover-up stock-out mt-2">Out
                                                            of stock !</button>
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            <div class="price-filter-inner">
                                <div class="price_slider_amount">
                                    <div class="label-input">
                                        <span>Range:</span><input type="text" class="js-range-slider" name="my_range"
                                            value="" />
                                    </div>
                                </div>
                            </div>
                        ...
                        @if (isset($categories))
                            @foreach ($categories as $category)
                                @if ($category->sub_category->isNotEmpty())
                                    <a class="" data-bs-toggle="collapse" href="#collapse{{ $category->id }}"
                                        role="button" aria-expanded="false" aria-controls="collapseExample">
                                        <p class="mt-2">{{ $category->name }}<i class="bi bi-chevron-down"
                                                style="margin-left: 30px"></i></p>
                                    </a>
                                @else
                                    <a href="{{ route('shop', $category->slug) }}">
                                        <p class="mt-2"
                                            style="{{ $categorySelected == $category->id ? 'color: #088178;' : '' }}">
                                            {{ $category->name }}</p>
                                    </a>
                                @endif
                                    ...
                                    <div class="">
                                        <ul class="categories">
                                            @foreach ($category->sub_category->where('status', 1) as $subCategory)
                                                <li><a href="{{ route('shop', [$category->slug, $subCategory->slug]) }}"
                                                        style="{{ $subCategorySelected == $subCategory->id ? 'color: #088178;' : '' }}">{{ $subCategory->name }}</a>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                                ...
                                @foreach ($brands as $brand)
                                    <li>
                                        <input {{ in_array($brand->id, $brandsArray) ? 'checked' : '' }}
                                            class="form-check-input brand-lable" type="checkbox" name="brand[]"
